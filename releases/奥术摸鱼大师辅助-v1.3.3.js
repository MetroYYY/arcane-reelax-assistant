...
(function() {
    ...
    const L = { map:logger('切图','#4a9eff'), reg:logger('报名','#4ade80'), buff:logger('Buff','#f59e0b'), bait:logger('鱼饵','#fbbf24'), fetch:logger('拦截','#9ca3af'), event:logger('事件','#c084fc'), cfg:logger('设置','#2dd4bf'), dlg:logger('弹窗','#f472b6'), reconn:logger('重连','#ef4444'), refill:logger('补杆','#a78bfa'), init:logger('主程序','#64748b') };
    ...
    bus.on('fishing:updated', ()=>{
        if(!settings.autoBuyBuffs && !settings.autoBait) return;
...
<aside class="dock" data-collapsed="false" aria-label="奥术摸鱼大师">
  ...
  <div class="panel-body">
    ...
    <div class="section">
      <div class="section-heading"><strong>换图优先级</strong></div>
    ...
    <div class="section">
      <div class="section-heading"><strong>自动切换鱼饵</strong><span>按场景选饵，切图时自动切换</span></div>
    ...
    function renderBaitControls(snapshot) {
        ...
        for (const scene of BAIT_SCENES) {
            ...
            for (const bait of (snapshot?.baits || [])) {
            ...
            sel.addEventListener('change', () => {
                ...
                if (chosenId && !settings.baitAutoBuy) {
                    const chosen = snapshot?.baits?.find(b => b.id === chosenId);
    ...
    async function main() {
        try {
            ...
            // 定时刷新面板信息
            setInterval(() => { if (state.appGame) { const s = state.appGame.getSnapshot(); updatePanelInfo(s); updateModeStatus(s); } }, 5000);
