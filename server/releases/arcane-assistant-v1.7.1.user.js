...
(function() {
    ...
    const UPDATE_NOTES = '【修复】\n- 掉竿统计：理论/掉竿与「今日 N 杆」不同步、掉竿每杆递增、理论比实际少却仍显示掉竿的问题\n- 鱼饵：没钱买饵时日志疯狂刷屏（降级尝试加 5 分钟冷却）\n- 鱼饵场景：比赛与天气同时出现时，先切天气饵再切回比赛饵的抖动\n- 赛后分配：开启自动切装备时，力量目标误用比赛幸运装计算导致偏大（改为先切回 1 号配装再算）\n- 自动加点：暂停后仍会自动分配属性\n- 问卷：同一问卷可跨版本重复提交（改为按问卷 ID 去重）\n\n【优化】\n- 「每日盈亏显示」开关更名「今日净赚/盈亏」，UI 标题与问号说明同步更新';
