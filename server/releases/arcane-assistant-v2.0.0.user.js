...
${body}`;
  ...
  function startMasterySacrifice() {
    if (state._masterySacrificeTimer || state.masterySacrificeRunning) return;
    checkMasterySacrifice("\u542F\u7528\u529F\u80FD");
    scheduleNextMasteryCheck();
  }
  function scheduleNextMasteryCheck() {
    if (state._masterySacrificeTimer || !settings.autoMasterySacrifice) return;
    const delay = MASTERY_CHECK_MIN_MS + Math.floor(Math.random() * (MASTERY_CHECK_MAX_MS - MASTERY_CHECK_MIN_MS + 1));
    state._masterySacrificeTimer = setTimeout(async () => {
      state._masterySacrificeTimer = null;
      await checkMasterySacrifice("\u5B9A\u65F6\u68C0\u67E5");
      scheduleNextMasteryCheck();
    }, delay);
  }
  function stopMasterySacrifice() {
    if (state._masterySacrificeTimer) clearTimeout(state._masterySacrificeTimer);
    state._masterySacrificeTimer = null;
    state._masterySacrificeLastLogSig = "";
  }
