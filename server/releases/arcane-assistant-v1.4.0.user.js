...
(function() {
    ...
    const DEFAULTS = {
        ...
        autoRespecPersonal: false, respecModePersonal: 'str1700',
        autoRespecGuild: false, respecModeGuild: 'str1700',
        sectionCollapsed: {},
    };
    ...
    function warn(...a) { console.warn(`${ts()} %c[辅助脚本]`,'color:#ef4444;font-weight:bold',...a); }
    function error(...a) { console.error(`${ts()} %c[辅助脚本]`,'color:#dc2626;font-weight:bold',...a); }
    // 按功能模块自动加前缀：const L = logger('切图'); L('消息'); → 14:30:05 [切图] 消息
    function logger(tag, color) { const style=`color:${color};font-weight:bold`; return (...a)=>{ if(settings.debugLog) console.log(`${ts()} %c[${tag}] ${a.join(' ')}`,style); }; }
    ...
    function calculateTotalExpBonus(b) {
        ...
        if (!settings.excludeMasteryBonus && typeof b.masteryExperienceBonusBasisPoints === 'number') t += b.masteryExperienceBonusBasisPoints;
        if (!settings.excludeGuildBoost && b.guildBoost && typeof b.guildBoost.experienceBonusBasisPoints === 'number') t += b.guildBoost.experienceBonusBasisPoints;
        return t;
    }
    ...
    async function autoRegisterPersonal() {
        ...
        for (const c of cand) {
            ...
            try {
                ...
                state.registeredPersonalIds.add(c.id); c.isRegistered = true; L.reg(`✅ #${c.sequence} 报名成功`);
    ...
    function makeDecision(game) {
        ...
        // 人在船队但离船了（手动个人切图了，船没动）→ 先归队
        if (snap.party?.isInParty && snap.currentBiomeId !== snap.party.boatBiomeId) {
            L.map(`离船 (${snap.currentBiomeId}→${snap.party.boatBiomeId})，自动归队`);
    ...
    window.fetch = async function(...args) {
        ...
        try {
            ...
            if(url.includes('/api/me')){
                ...
                if(d.player?.stats)updateState({playerStats:d.player.stats});
            }
    ...
    bus.on('fishing:updated', ()=>{
        if(!settings.autoBuyBuffs && !settings.autoBait && !settings.autoAllocateStats && !settings.autoRespecPersonal && !settings.autoRespecGuild) return;
    ...
    function applySettings() {
        ...
        if(settings.autoAllocateStats && state.unspentStatPoints > 0) autoAllocateStats();
        // 洗点开关变更 → 关则洗回，开则检查是否需要洗
        const backup = JSON.parse(localStorage.getItem(RESPEC_BACKUP_KEY) || '{}');
        if (backup.personal && !settings.autoRespecPersonal) restoreRespec('personal');
        if (backup.guild && !settings.autoRespecGuild) restoreRespec('guild');
        if (settings.autoRespecPersonal || settings.autoRespecGuild) checkRespecRestore();
    }
...
<style>
  ...
  *,*::before,*::after{box-sizing:border-box;}
  ...
  .dock[data-collapsed="true"] .icon-button{width:48px;height:48px;border-radius:50%;background-image:url(https://reelax.cn/icons/currency/gold-coin.webp);background-size:cover;background-position:center;}
...
<aside class="dock" data-collapsed="false" aria-label="奥术摸鱼大师">
  ...
  <div class="panel-body">
    ...
    function renderStatsSection() {
        ...
        spanA.append(document.createTextNode('自动加点'), statSel);
        ...
        (()=>{
            ...
            sel.style.cssText = 'width:130px;height:24px;border:1px solid var(--as-border);border-radius:3px;background:var(--as-raised);color:var(--as-text);font:inherit;font-size:11px;';
            for (const [v, t] of [['str1700','力1700其余幸运'],['str3800','力3800其余幸运'],['str5400','力5400其余幸运'],['all','全幸运']]) {
                const o = document.createElement('option'); o.value = v; o.textContent = t; if (settings.respecModePersonal === v) o.selected = true;
                sel.appendChild(o);
            }
            sel.addEventListener('change', () => { settings.respecModePersonal = sel.value; saveSettings(); });
            const span = document.createElement('span'); span.style.whiteSpace = 'nowrap';
            span.append(document.createTextNode('个人赛洗点'), sel);
            ...
            row.appendChild(span); row.appendChild(cb); ctr.appendChild(row);
        })();
        // 公会赛洗点
        (()=>{
            const row = document.createElement('label'); row.className = 'switch-item';
            ...
            sel.style.cssText = 'width:130px;height:24px;border:1px solid var(--as-border);border-radius:3px;background:var(--as-raised);color:var(--as-text);font:inherit;font-size:11px;';
            for (const [v, t] of [['str1700','力1700其余幸运'],['str3800','力3800其余幸运'],['str5400','力5400其余幸运'],['all','全幸运']]) {
                const o = document.createElement('option'); o.value = v; o.textContent = t; if (settings.respecModeGuild === v) o.selected = true;
                sel.appendChild(o);
            }
            sel.addEventListener('change', () => { settings.respecModeGuild = sel.value; saveSettings(); });
            const span = document.createElement('span'); span.style.whiteSpace = 'nowrap';
            span.append(document.createTextNode('公会赛洗点'), sel);
    ...
    function attachUI() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (!e.composedPath().includes(host)) {
                ...
                if (collapse) { collapse.setAttribute('aria-expanded', 'false'); collapse.querySelector('.collapse-glyph').textContent = '+'; collapse.title = '点击展开辅助脚本设置'; }
                saveSettings();
            }
        });
    }
    ...
    function checkBaitScene() {
        ...
        if (state.lastBaitScene && state.lastBaitScene !== scene) {
            ...
            // 比赛结束 → 恢复洗点
            if (oldScene === 'personalCompetition') restoreRespec('personal');
            else if (oldScene === 'guildCompetition') restoreRespec('guild');
            // 鱼饵切换
    ...
    async function autoAllocateStats() {
        ...
        try {
            ...
            if (r.player?.unspentStatPoints !== undefined) {
                ...
                console.log(`${ts()} %c[加点]%c ✅ 自动分配 ${pts} 点 → ${STAT_LABELS[settings.statAllocationTarget]||settings.statAllocationTarget}`,'color:#f59e0b;font-weight:bold','');
            }
        } catch(e) {
            console.error(`${ts()} %c[加点]%c 分配失败: ${e.message}`,'color:#f59e0b;font-weight:bold','');
        } finally { updateState({ statAllocateInProgress: false }); }
    }

    const RESPEC_BACKUP_KEY = 'arcane_respec_backup';
    const RESPEC_COST = 10000;
    const RESPEC_STRENGTHS = { str1700:1700, str3800:3800, str5400:5400 };
    const INIT_ENDURANCE = 100;

    function respecEnabled(kind) {
        return kind === 'personal' ? settings.autoRespecPersonal : settings.autoRespecGuild;
    }
    function respecMode(kind) {
        return kind === 'personal' ? settings.respecModePersonal : settings.respecModeGuild;
    }

    async function doRespec(kind) {
        if (state.respecInProgress) return;
        if (!respecEnabled(kind)) return;
        const mode = respecMode(kind);
        const label = kind === 'personal' ? '个人赛' : '公会赛';
        if (state.playerGold < RESPEC_COST) { console.warn(`${ts()} %c[洗点]%c ${label}金币不足 (${state.playerGold} < ${RESPEC_COST})`,'color:#f97316;font-weight:bold',''); return; }
        const backup = JSON.parse(localStorage.getItem(RESPEC_BACKUP_KEY) || '{}');
        if (backup[kind]) { L.spc(`${label}已存在备份，跳过`); return; }
        if (!state.playerStats) { L.spc('等待属性数据'); return; }
        state.respecInProgress = true;
        try {
            const st = state.playerStats;
            const cc = state.competitionCache;
            const compId = kind==='personal' ? cc.personal?.current?.id : cc.guild?.current?.id;
            backup[kind] = { base: { ...st.base }, unspent: state.unspentStatPoints, competitionId: compId };
            localStorage.setItem(RESPEC_BACKUP_KEY, JSON.stringify(backup));
            const targetStr = RESPEC_STRENGTHS[mode] || 0;
            const keepStr = targetStr > 0;
            console.log(`${ts()} %c[洗点]%c ${label}开始 → ${keepStr?'总计力'+targetStr+'其余幸运':'全加幸运'}`,'color:#f97316;font-weight:bold','');
            const resetR = await apiFetch('/api/player/stats/reset', { method:'POST', idempotencyKey: crypto.randomUUID() });
            if (resetR.player) updateState({ playerGold: resetR.player.gold, unspentStatPoints: resetR.player.unspentStatPoints, playerStats: resetR.player.stats });
            const totalPts = resetR.player?.unspentStatPoints ?? 0;
            const flatBonus = resetR.player?.stats?.total?.strength || 0;
            const preBase = st.base?.strength || 0;
            const preTotal = st.total?.strength || 0;
            const mult = preBase > 0 ? ((preTotal - flatBonus) / preBase) : 1; // 基础属性倍率
            ...
            body.luck = totalPts - body.strength - INIT_ENDURANCE;
            if (body.luck < 0) body.luck = 0;
            const allocR = await apiFetch('/api/player/stats/allocate', { method:'POST', body, idempotencyKey: crypto.randomUUID() });
            if (allocR.player) updateState({ playerGold: allocR.player.gold, unspentStatPoints: allocR.player.unspentStatPoints, playerStats: allocR.player.stats });
            console.log(`${ts()} %c[洗点]%c ✅ ${label}洗点完成: 外部加成${flatBonus} 倍率${mult.toFixed(3)} → 基础力${body.strength} 智0 运${body.luck} 耐${INIT_ENDURANCE}`,'color:#f97316;font-weight:bold','');
        } catch(e) {
            console.error(`${ts()} %c[洗点]%c ${label}失败: ${e.message}`,'color:#f97316;font-weight:bold','');
            // 失败则清除刚才存的备份避免残留
            const b2 = JSON.parse(localStorage.getItem(RESPEC_BACKUP_KEY) || '{}');
            delete b2[kind]; localStorage.setItem(RESPEC_BACKUP_KEY, JSON.stringify(b2));
        } finally { updateState({ respecInProgress: false }); }
    }

    async function restoreRespec(kind) {
        if (state.respecInProgress) return;
        const backup = JSON.parse(localStorage.getItem(RESPEC_BACKUP_KEY) || '{}');
        const saved = backup[kind]; if (!saved) return;
        const label = kind === 'personal' ? '个人赛' : '公会赛';
        state.respecInProgress = true;
        try {
            console.log(`${ts()} %c[洗点]%c 恢复${label}前属性`,'color:#f97316;font-weight:bold','');
            const resetR = await apiFetch('/api/player/stats/reset', { method:'POST', idempotencyKey: crypto.randomUUID() });
            const avail = resetR.player?.unspentStatPoints ?? 0;
            const body = { ...saved.base };
            const sum = body.strength + body.intelligence + body.luck + body.endurance;
            // 自适应入池和出池差异
            if (sum > avail) { const s = avail / sum; body.strength = Math.floor(body.strength * s); body.intelligence = Math.floor(body.intelligence * s); body.luck = Math.floor(body.luck * s); body.endurance = Math.max(INIT_ENDURANCE, Math.floor(body.endurance * s)); }
            else if (sum < avail) { body.luck += (avail - sum); }
            const allocR = await apiFetch('/api/player/stats/allocate', { method:'POST', body, idempotencyKey: crypto.randomUUID() });
            if (allocR.player) updateState({ playerGold: allocR.player.gold, unspentStatPoints: allocR.player.unspentStatPoints, playerStats: allocR.player.stats });
            if (settings.autoAllocateStats && state.unspentStatPoints > 0) autoAllocateStats();
            delete backup[kind]; localStorage.setItem(RESPEC_BACKUP_KEY, JSON.stringify(backup));
            console.log(`${ts()} %c[洗点]%c ✅ ${label}属性已恢复`,'color:#f97316;font-weight:bold','');
        } catch(e) {
            console.error(`${ts()} %c[洗点]%c 恢复${label}失败: ${e.message}，下次初始化重试`,'color:#f97316;font-weight:bold','');
        } finally { updateState({ respecInProgress: false }); }
    }

    function checkRespecRestore() {
        const backup = JSON.parse(localStorage.getItem(RESPEC_BACKUP_KEY) || '{}');
        const p = state.competitionCache.personal;
        const g = state.competitionCache.guild;
        const now = Date.now();
        // 恢复：比赛结束了但还没洗回
        if (backup.personal) {
            const inProgress = (p?.current?.isRegistered && now <= new Date(p.current.endAt).getTime())
                || (p?.upcoming?.some(c => c.isRegistered && now >= new Date(c.startAt).getTime() - 300000));
            if (!inProgress) restoreRespec('personal');
        }
        if (backup.guild) {
            const inProgress = (g?.current?.entryStatus === 'registered' && now <= new Date(g.current.endAt).getTime())
                || (g?.upcoming?.some(c => c.entryStatus === 'registered' && now >= new Date(c.startAt).getTime() - 300000));
            if (!inProgress) restoreRespec('guild');
        }
        // 刷新后已在比赛中：没备份 或 备份是上一场的 → 洗点
        const personalActive = respecEnabled('personal') && p?.current?.isRegistered
            && now <= new Date(p.current.endAt).getTime();
        const guildActive = respecEnabled('guild') && g?.current?.entryStatus === 'registered'
            && now <= new Date(g.current.endAt).getTime();
        if (personalActive && (!backup.personal || backup.personal.competitionId !== p.current.id)) {
            if (backup.personal) { L.spc('清除陈旧个人赛备份'); delete backup.personal; localStorage.setItem(RESPEC_BACKUP_KEY, JSON.stringify(backup)); }
            doRespec('personal');
        }
        if (guildActive && (!backup.guild || backup.guild.competitionId !== g.current.id)) {
            if (backup.guild) { L.spc('清除陈旧公会赛备份'); delete backup.guild; localStorage.setItem(RESPEC_BACKUP_KEY, JSON.stringify(backup)); }
            doRespec('guild');
        }
    }
    ...
    async function main() {
        try {
            ...
            game.on('competition:started', ({competition}) => { if(settings.autoSwitchMap)makeDecision(game); evaluateBait(); doRespec(competition.kind); });
            ...
            // 定时刷新面板信息
            setInterval(() => { if (state.appGame) { const s = state.appGame.getSnapshot(); updatePanelInfo(s); updateModeStatus(s); renderBaitControls(s); checkBaitScene(); } }, 5000);
