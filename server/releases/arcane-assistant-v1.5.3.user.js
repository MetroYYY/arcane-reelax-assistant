...
(function() {
    ...
    const DEFAULTS = {
        ...
        sectionCollapsed: {},
        viewMode: 'settings', paused: false, showPity: true,
    };
    ...
    async function apiFetch(path, opts = {}) {
        ...
        if (!r.ok) {
            let msg = `${r.status}`; try { const e = await r.clone().json(); if (e.error?.message) msg = e.error.message; } catch(_) {}
            // 签名过期 → 清缓存重试一次
            if (msg.includes('签名') && !opts._retried) {
    ...
    // 注入保底区块到自动钓鱼面板标题下方（两行，标题+奇异+奥秘横向排列，渐变字体）
    function injectPityPanel() {
        ...
        if (!el) {
            ...
            el.style.cssText = 'display:grid;grid-template-columns:auto auto auto;gap:2px 16px;padding:8px 12px;border-bottom:1px solid var(--divider,#e4edf2);font-family:inherit;';
            heading.insertAdjacentElement('afterend', el);
        }
        const title = (t) => `<span style="font-size:11px;line-height:18px;color:var(--muted,#71869b);white-space:nowrap">${t} ：</span>`;
        const data = (key, name, mode) => {
            ...
            return `<span style="font-size:12px;line-height:18px;font-weight:600;background:${g};-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;color:transparent;white-space:nowrap">${name}：${text}</span>`;
        };
        ...
        for (const pt of priority) {
            ...
            case 'competition': {
                ...
                if (ct && settings.autoPersonal) {
                    const seq = getPersonalCompContext()?.sequence || '';
                    if (settings.witherTideDipPersonal && state._witherDipSeq !== seq && ct.weather?.id === 'wither_tide')
    ...
    window.fetch = async function(...args) {
        ...
        try {
            ...
            if(url.includes('/api/fishing/state')||url.includes('/api/fishing/sync')){
                ...
                if(url.includes('/api/fishing/sync') && settings.showPity && state._pityLoaded){ trackPityCast(d?.lastResult?.rarity); injectPityPanel(); }
...
<aside class="dock" data-collapsed="false" aria-label="奥术摸鱼大师">
  ...
  <div class="panel-body">
    ...
    function checkBaitScene() {
        ...
        if (state.lastBaitScene && state.lastBaitScene !== scene) {
            ...
            if ((oldScene === 'personalCompetition' || oldScene === 'guildCompetition')
                ...
                if (settings.autoRespecPersonal || settings.autoRespecGuild) applyPostRespec();
    ...
    function statsMatchPostRespec() {
        ...
        const strengthMaxedOut = targetStr > 0 && b.intelligence === 0 && b.luck === 0;
        // 力量目标不达标 → 不匹配（除非已打满，说明目标不可达）
        if (targetStr > 0 && !strengthMaxedOut && t.strength < targetStr) return false;
    ...
    async function main() {
        try {
            ...
            game.on('weather:changed', ({biomeId,previous,current}) => { updateState({currentWeatherId:current.id}); updatePanelInfo(game.getSnapshot()); if (settings.showPity) { L.pity(`天气变化 ${previous?.id}→${current.id}，重新校准`); fetchPity(); } if (state.paused) return; if(settings.autoBuyBuffs)checkAndBuyBuffs(); if(settings.autoSwitchMap)makeDecision(game); evaluateBait(); });
